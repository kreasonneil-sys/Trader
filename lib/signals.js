...
function runStrategy(aligned, regimes, rsiAligned, macdHist, atrAligned, adxAligned, ticker) {
  ...
  // Forward pass — position: 1 = long, -1 = short, 0 = flat
  const stratEquity = [10000];
  let position = 0;
  const trades = [];
  let entryPrice = 0, entryIdx = 0, trailingHigh = 0, trailingLow = Infinity;
  let cooldownUntil = 0;
  ...
  for (let t = 1; t < len; t++) {
    ...
    const trendDown = ema1[t] < ema2[t] && ema2[t] < ema3[t];
    const trendConfirmed = ema1[t] > ema3[t];
    const trendBearish = ema1[t] < ema3[t];

    // --- EXIT LOGIC ---
    let forceExitLong = false;
    let forceExitShort = false;

    if (position === 1) {
      if (close > trailingHigh) trailingHigh = close;
      if (close < trailingHigh - profile.chandelierMult * curATR) forceExitLong = true;
      const exitDonchIdx = t - exitOffset;
      if (exitDonchIdx >= 0 && exitDonchIdx < exitDonch.lower.length) {
        if (close < exitDonch.lower[exitDonchIdx]) forceExitLong = true;
      }
      if (ema1[t] < ema3[t] && ema1[t - 1] >= ema3[t - 1]) forceExitLong = true;
    }

    if (position === -1) {
      if (close < trailingLow) trailingLow = close;
      // Cover short: chandelier (price rises M× ATR from trailing low)
      if (close > trailingLow + profile.chandelierMult * curATR) forceExitShort = true;
      // Cover short: price breaks above exit Donchian high
      const exitDonchIdx = t - exitOffset;
      if (exitDonchIdx >= 0 && exitDonchIdx < exitDonch.upper.length) {
        if (close > exitDonch.upper[exitDonchIdx]) forceExitShort = true;
      }
      // Cover short: trend reversal — fast EMA crosses above slow EMA
      if (ema1[t] > ema3[t] && ema1[t - 1] <= ema3[t - 1]) forceExitShort = true;
    }
    ...
    if (canEnter) {
      ...
      if (!enterLong && profile.pullbackReentry && trendUp) {
        ...
        if (nearEMA && bouncing && rsiOk) enterLong = true;
      }

      if (!enterLong && trendConfirmed && curADX > (profile.macdAdxThresh || 25)) {
        const macdNow = macdHist[t] ?? 0;
        const macdPrev = macdHist[t - 1] ?? 0;
        if (macdNow > 0 && macdPrev <= 0 && close > ema2[t]) enterLong = true;
      }

      // Short entries — mirror of long logic in downtrend
      if (!enterLong) {
        const trendReqShort = profile.requireStrongTrend ? trendDown : trendBearish;
        const rsiReqShort = curRSI <= (100 - (profile.breakoutRsiMin || 0));
        // Short 1: Breakdown — new Donchian low in downtrend
        if (entryDonchIdx >= 0 && entryDonchIdx < entryDonch.lower.length) {
          if (close < entryDonch.lower[entryDonchIdx] && trendReqShort && rsiReqShort) {
            enterShort = true;
          }
        }

        // Short 2: Rally-to-sell — price rallies to EMA then fails (mirror of pullback)
        if (!enterShort && profile.pullbackReentry && trendDown) {
          const pbZone = profile.pullbackZone || 0.03;
          const nearEMA = close >= ema2[t] * (1 - pbZone * 0.33) && close <= ema2[t] * (1 + pbZone);
          const failing = aligned[t].close < aligned[t].open
                       && (t >= 2 && aligned[t - 1].close < aligned[t - 1].open);
          const pbRsiLo = 100 - (profile.pullbackRsiHi || 65);
          const pbRsiHi = 100 - (profile.pullbackRsiLo || 40);
          const rsiOk = curRSI > pbRsiLo && curRSI < pbRsiHi;
          if (nearEMA && failing && rsiOk) enterShort = true;
        }

        // Short 3: MACD crossing negative with ADX strength
        if (!enterShort && trendBearish && curADX > (profile.macdAdxThresh || 25)) {
          const macdNow = macdHist[t] ?? 0;
          const macdPrev = macdHist[t - 1] ?? 0;
          if (macdNow < 0 && macdPrev >= 0 && close < ema2[t]) enterShort = true;
        }
      }
    }
    ...
    if (forceExitShort && position === -1) {
      const retPct = ((entryPrice - close) / entryPrice) * 100;
      position = 0;
      trades.push({
        entryDate: aligned[entryIdx].date,
        exitDate: aligned[t].date,
    ...
    // Equity: positionAtOpen determines exposure direction
    const dailyRet = (close / prevClose) - 1;
    stratEquity.push(stratEquity[stratEquity.length - 1] * (1 + dailyRet * positionAtOpen));
  }
  ...
  if (position === 1) {
    ...
    trades.push({
      entryDate: aligned[entryIdx].date, exitDate: null,
      entryPrice, exitPrice: lastClose,
      returnPct: retPct, holdDays: (len - 1) - entryIdx,
      side: 'LONG', open: true
    });
  } else if (position === -1) {
    const lastClose = aligned[len - 1].close;
    const retPct = ((entryPrice - lastClose) / entryPrice) * 100;
    trades.push({
      entryDate: aligned[entryIdx].date, exitDate: null,
      entryPrice, exitPrice: lastClose,
      returnPct: retPct, holdDays: (len - 1) - entryIdx,
      side: 'SHORT', open: true
    });
  }
...
function runBounceStrategy(aligned, rsiAligned, atrAligned) {
  ...
  for (let t = 1; t < len; t++) {
    ...
    if (position === 1) {
      ...
      if (maVal != null && close < maVal) {
        failCount++;
        if (failCount >= failBars) forceExitLong = true;
      } else {
        failCount = 0;
      }
      if (close < trailingHigh - chandelierMult * curATR) forceExitLong = true;
    }

    if (position === -1) {
      if (close < trailingLow) trailingLow = close;
      // Cover short: price closes above the rejection MA
      const maVal = mas[bounceMA] ? mas[bounceMA][t] : null;
      if (maVal != null && close > maVal) {
        failCount++;
        if (failCount >= failBars) forceExitShort = true;
      } else {
        failCount = 0;
      }
      // Cover short: chandelier (price rises M× ATR from trailing low)
      if (close > trailingLow + chandelierMult * curATR) forceExitShort = true;
    }
    ...
    if (position === 0 && t >= cooldownUntil) {
      ...
      // Short: rejection off MA — price rallied to MA from below then failed
      if (!enterLong) {
        for (const m of BOUNCE_LEVELS) {
          const maVal = mas[m.key] ? mas[m.key][t] : null;
          const maPrev = mas[m.key] ? mas[m.key][t - 1] : null;
          ...
          // Rejection: yesterday's high touched/pierced MA zone from below, today closes below
          const touchedMA = aligned[t - 1].high >= maPrev * (1 - m.touchZone);
          const rejectedBelow = close < maVal;
          const bearishCandle = close < aligned[t].open;
          const curRSI = rsiAligned[t] ?? 50;
          const rsiHigh = curRSI > 45; // not oversold — genuine rejection, not capitulation
          const candleRange = aligned[t].high - aligned[t].low;
          const candleBody = aligned[t].open - close;
          const strongReject = candleRange > 0 ? (candleBody / candleRange) > 0.3 : false;

          if (touchedMA && rejectedBelow && bearishCandle && rsiHigh && strongReject) {
            enterShort = true;
            enterMA = m.key;
            break;
          }
    ...
    if (forceExitLong && position === 1) {
      ...
      position = 0;
      trades.push({
        entryDate: aligned[entryIdx].date, exitDate: aligned[t].date,
        entryPrice, exitPrice: close, returnPct: retPct,
        holdDays: t - entryIdx, bounceLevel: bounceMA, side: 'LONG'
      });
      bounceMA = null; failCount = 0;
    ...
    if (forceExitShort && position === -1) {
      ...
      trades.push({
        entryDate: aligned[entryIdx].date, exitDate: aligned[t].date,
        entryPrice, exitPrice: close, returnPct: retPct,
        holdDays: t - entryIdx, bounceLevel: bounceMA, side: 'SHORT'
      });
      bounceMA = null; failCount = 0;
      cooldownUntil = t + cooldownBars;
      lastAction = 2;
    ...
    if (enterLong && position === 0) {
      position = 1;
      entryPrice = close; entryIdx = t;
      trailingHigh = close; bounceMA = enterMA;
      failCount = 0; lastAction = 1;
    }
    if (enterShort && position === 0) {
      position = -1;
      entryPrice = close; entryIdx = t;
      trailingLow = close; bounceMA = enterMA;
      failCount = 0; lastAction = 1;
    }
  ...
  if (position === 1) {
    ...
    trades.push({
      entryDate: aligned[entryIdx].date, exitDate: null,
      entryPrice, exitPrice: lastClose, returnPct: retPct,
      holdDays: (len - 1) - entryIdx, bounceLevel: bounceMA,
      side: 'LONG', open: true
    });
  } else if (position === -1) {
    const lastClose = aligned[len - 1].close;
