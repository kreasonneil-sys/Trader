...
const ASSET_PROFILES = {
  gold: {
    // Gold trends persistently — wider RSI bands, wider stops, more patience
    rsiBuckets: [35, 75],        // RSI often stays high in gold bull runs
    txCost: 0.0005,              // Futures have tight spreads
    episodes: 800,               // More training for gold's nuanced patterns
    ensembleSize: 5,             // More ensemble members for stability
    lossPenalty: 2.0,            // Penalize losses harder — gold drawdowns are sharp
    trendBonus: 0.35,            // Strong trend-following bonus for gold momentum
    trailBull: 3.0,              // Wide stop in bull: let gold run (momentum persistence)
    trailBullWeak: 2.5,          // Moderate stop in weak bull
    trailBear: 1.8,              // Tighter in bear but not too tight (gold is volatile)
    adxEntry: 18,                // Lower ADX threshold — gold trends at lower ADX
    cooldownLoss: 3,             // Shorter cooldown — gold recoveries are fast
    cooldownWin: 1,              // Minimal cooldown after wins
    maxEquityDD: 12,             // Slightly wider DD tolerance for gold volatility
    tpMultiplier: 3.0,           // 3:1 R:R for gold (trends run far)
    slMultiplier: 1.0,           // Tight SL at 1× ATR
  },
  default: {
    rsiBuckets: [40, 60],
    txCost: 0.0015,
...
// --- RL Q-Learning (Ensemble) ---

function trainQTable(states, aligned, regimes, profile, nStates, len) {
  const Q = Array.from({ length: nStates }, () => Array(3).fill(0));
  const alpha = 0.15, gamma = 0.95;

  for (let ep = 0; ep < profile.episodes; ep++) {
    const epsilon = Math.max(0.01, 0.3 * Math.exp(-ep / 100));
    let position = 0;
    for (let t = 1; t < len - 1; t++) {
      ...
      let reward = 0;
      if (action === 1 && position === 0) { position = 1; reward -= profile.txCost; }
      if (action === 2 && position === 1) { position = 0; reward -= profile.txCost; }
      const dailyRet = (aligned[t + 1].close / aligned[t].close) - 1;
      // Asymmetric reward with asset-specific loss penalty
      const shaped = dailyRet >= 0 ? dailyRet : dailyRet * profile.lossPenalty;
      const trendBonus = regimes[t] === 2 && dailyRet > 0 ? dailyRet * profile.trendBonus : 0;
      reward += shaped * position + trendBonus * position;
...
function runQLearning(aligned, regimes, rsiAligned, macdHist, atrAligned, adxAligned, ticker) {
  ...
  const states = regimes.map((reg, i) => {
    ...
    // Asset-specific RSI buckets (gold uses wider bands to avoid cutting winners)
    const rsiBucket = r < profile.rsiBuckets[0] ? 0 : r > profile.rsiBuckets[1] ? 2 : 1;
  ...
  // Ensemble: train N Q-tables and average for stability
  const Qtables = [];
  for (let n = 0; n < profile.ensembleSize; n++) {
    Qtables.push(trainQTable(states, aligned, regimes, profile, nStates, len));
  }
  const Q = Array.from({ length: nStates }, (_, s) =>
    Array.from({ length: nActions }, (_, a) =>
      Qtables.reduce((sum, qt) => sum + qt[s][a], 0) / profile.ensembleSize
    )
  );

  // Forward pass with regime-adaptive trailing stop, cooldown, and drawdown circuit breaker
  ...
  for (let t = 1; t < len; t++) {
    ...
    const curEquity = rlEquity[rlEquity.length - 1];
    if (curEquity > equityPeak) equityPeak = curEquity;
    const equityDD = ((equityPeak - curEquity) / equityPeak) * 100;

    // Regime-adaptive trailing stop with asset-specific multipliers
    const isBull = regimes[t] === 2;
    const isStrongTrend = curADX >= 25;
    const trailMult = isBull && isStrongTrend ? profile.trailBull
                    : isBull ? profile.trailBullWeak
                    : profile.trailBear;
    ...
    if (position === 1) {
      ...
      if (aligned[t].close < trailingHigh - trailMult * curATR) forceExit = true;
    }

    // Entry conditions with asset-specific thresholds
    const canEnter = t >= cooldownUntil && curADX > profile.adxEntry && equityDD < profile.maxEquityDD;
    if (action === 1 && position === 0 && canEnter) {
      position = 1;
    ...
    if ((action === 2 || forceExit) && position === 1) {
      const retPct = ((aligned[t].close - entryPrice) / entryPrice) * 100;
      position = 0;
      trades.push({
      ...
      if (forceExit || retPct < 0) cooldownUntil = t + profile.cooldownLoss;
      else cooldownUntil = t + profile.cooldownWin;
    }
...
export async function fetchSignals(tickerCode = 'STX40.JO') {
  ...
  // RL Q-Learning (expanded state space with MACD + volatility + ADX gate)
  const { Q, states, rlEquity, trades } = runQLearning(aligned, regimes, rsiAligned, macdAligned.histogram, atrAligned, adxAligned, tickerCode);
  ...
  // Asset-aware RSI filter: gold allows RSI up to 75 (trends persist at high RSI)
  const rsiLow = profile.rsiBuckets[0];
  const rsiHigh = profile.rsiBuckets[1];
  factors.push({
    name: 'RSI Filter',
  ...
  // ADX trend quality: asset-specific threshold (gold trends at lower ADX values)
  const adxThreshold = profile.adxEntry + 2; // confluence uses slightly higher bar than entry gate
  factors.push({
    name: 'ADX Trend Quality',
    description: `ADX above ${adxThreshold} (strong directional trend)`,
    passed: latestADX >= adxThreshold,
  ...
  if (confluence) {
    ...
    for (let i = 0; i < alignedLen - 30; i++) {
      ...
      const pastADXStrong = (adxAligned[i] ?? 20) >= 25;
      const pastScore = [pastBull, pastRSI >= rsiLow && pastRSI <= rsiHigh, pastMACDPos, pastBBOk, pastAboveSMA, pastADXStrong].filter(Boolean).length;
      if (pastScore >= 5) {
        total++;
