...
async function fetchSignals() {
  ...
  // Fetch Satrix 40 (primary instrument)
  const satrixRaw = await yahooFinance.historical('STX40.JO', {
    period1: start, period2: end, interval: '1d'
  });

  const satrix = satrixRaw.map(h => ({
  ...
  if (satrix.length < 200) throw new Error('Not enough historical data');
  ...
  // Regime detection (SMA crossover proxy for HMM)
  ...
  if (confluence) {
    ...
    // Historical win probability backtest
    let wins = 0, total = 0;
  ...
  // Prepare chart data (last 252 trading days)
  ...
  // Fetch top 40 individual stock latest prices
  const top40Data = [];
  for (const code of TOP_40_CODES.slice(0, 10)) {
    try {
      const quote = await yahooFinance.quote(`${code}.JO`);
      top40Data.push({
        code,
