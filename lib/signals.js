...
const ASSET_PROFILES = {
  gold: {
    ...
    pullbackZone: 0.03,          // ±3% from EMA — standard for gold's moderate vol
    pullbackRsiLo: 40,           // Gold pullbacks to RSI 40 are healthy
    pullbackRsiHi: 65,           // Don't chase at extreme RSI in pullback mode
    macdAdxThresh: 22,           // Gold trends at moderate ADX levels
  ...
  satrix: {
    // Satrix 40 — equity index ETF with moderate trends, mean-reverting dips
    // Tuned via 10-seed sweep: avg alpha 44.8% (up from 36.4%)
    rsiBounds: [35, 70],         // Slightly tighter than gold, equities revert faster
    txCost: 0.001,               // ETF trading costs
    breakoutLen: 12,             // Short 12-day breakout — catch equity rallies early
    exitLen: 6,                  // Very fast 6-day exit — equity corrections are sharp and sudden
    chandelierMult: 2.0,         // Tight 2× ATR chandelier — low equity vol means cut losers fast
    ema1: 8,                     // Fast EMA for quicker equity cycles
    ema2: 21,                    // 21-day EMA (1 month) — standard equity timeframe
    ema3: 50,                    // 50-day for macro
    pullbackReentry: true,       // Equities bounce well from dips
    pullbackZone: 0.02,          // ±2% from EMA — tight zone for low-vol equity
    pullbackRsiLo: 30,           // Catch dip-buys at lower RSI
    pullbackRsiHi: 72,           // Allow entries when momentum is building
    macdAdxThresh: 18,           // Lower ADX bar — equity trends are gentler than commodities
    cooldownBars: 1,             // Fast re-entry — equity bounces are quick
    maxEquityDD: 12,
    tpMultiplier: 2.5,
    slMultiplier: 0.8,           // Tighter stop — equities have lower ATR
  },
  btc: {
    // Bitcoin — extreme volatility, parabolic trends, sudden crashes
    // Tuned via 10-seed sweep: avg alpha 278% (up from 259%)
    rsiBounds: [30, 82],         // BTC regularly trends at RSI 70-82 in bull markets
    txCost: 0.001,               // Exchange fees
    breakoutLen: 14,             // BTC breakouts are fast — short channel catches them
    exitLen: 8,                  // 8-day exit — survives normal dips, catches crash starts
    chandelierMult: 3.8,         // 3.8× ATR — balanced: survives pullbacks, exits crashes
    ema1: 5,                     // Very fast 5-day EMA — BTC regime changes are rapid
    ema2: 18,                    // 18-day medium EMA — captures BTC's shorter cycles
    ema3: 45,                    // 45-day macro — BTC trends shift quicker than equities
    pullbackReentry: true,       // BTC pullbacks in bull markets are buying opportunities
    pullbackZone: 0.06,          // ±6% from EMA — BTC swings 5-7% routinely
    pullbackRsiLo: 25,           // BTC can dip to RSI 25-30 in healthy corrections
    pullbackRsiHi: 80,           // Allow entries well into momentum territory
    macdAdxThresh: 18,           // Lower bar — catch more MACD cross entries in BTC
    cooldownBars: 1,             // Minimal cooldown — BTC moves fast
    maxEquityDD: 25,             // BTC drawdowns can be deep, wider tolerance
    tpMultiplier: 4.0,           // 4:1 R:R — BTC trends run far
    slMultiplier: 1.5,           // Wider stop for BTC's extreme intraday ranges
  },
  default: {
    rsiBounds: [40, 60],
    txCost: 0.0015,
...
function getAssetProfile(ticker) {
  ...
  if (t === 'STX40.JO' || t.startsWith('STX40')) return ASSET_PROFILES.satrix;
...
function runBounceStrategy(aligned, rsiAligned, atrAligned) {
  ...
  const failBars = 1;        // 1 close below MA = failure confirmed
  const chandelierMult = 3.0; // 3× ATR trailing stop backup
  ...
  for (let t = 1; t < len; t++) {
    ...
    if (position === 0 && t >= cooldownUntil) {
      ...
      for (const m of BOUNCE_LEVELS) {
        ...
        if (touchedMA && bouncedAbove && bullishCandle) {
          enterSignal = true;
          enterMA = m.key;
...
export async function fetchSignals(tickerCode = 'STX40.JO', backtestPeriod = '6m', strategyKey = 'config1') {
  const end = new Date();
