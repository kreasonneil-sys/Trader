...
function trainQTable(states, aligned, regimes, txCost, nStates, len) {
  ...
  for (let ep = 0; ep < 500; ep++) {
    ...
    for (let t = 1; t < len - 1; t++) {
      ...
      // Asymmetric reward: penalize losses 1.5× harder than rewarding wins
      const shaped = dailyRet >= 0 ? dailyRet : dailyRet * 1.5;
      const trendBonus = regimes[t] === 2 && dailyRet > 0 ? dailyRet * 0.25 : 0;
...
function runQLearning(aligned, regimes, rsiAligned, macdHist, atrAligned, adxAligned) {
  ...
  // Ensemble: train 3 Q-tables and average them to reduce randomness
  const Q1 = trainQTable(states, aligned, regimes, txCost, nStates, len);
  const Q2 = trainQTable(states, aligned, regimes, txCost, nStates, len);
  const Q3 = trainQTable(states, aligned, regimes, txCost, nStates, len);
  const Q = Array.from({ length: nStates }, (_, s) =>
    Array.from({ length: nActions }, (_, a) => (Q1[s][a] + Q2[s][a] + Q3[s][a]) / 3)
  );
  ...
  let cooldownUntil = 0;         // re-entry cooldown after stop-loss
  let equityPeak = 10000;        // for drawdown circuit breaker
  ...
  for (let t = 1; t < len; t++) {
    ...
    // Bull with strong trend: wider (2.5× ATR), choppy/bear: tighter (1.5× ATR)
    const isBull = regimes[t] === 2;
    const isStrongTrend = curADX >= 25;
    const trailMult = isBull && isStrongTrend ? 2.5 : isBull ? 2.0 : 1.5;
    ...
    // Entry conditions: RL says BUY + cooldown elapsed + ADX > 20 (trending) + equity DD < 10%
    const canEnter = t >= cooldownUntil && curADX > 20 && equityDD < 10;
    ...
    if ((action === 2 || forceExit) && position === 1) {
      ...
      // Cooldown: 5 bars after a losing trade, 2 bars after a stop-loss exit
      if (forceExit || retPct < 0) cooldownUntil = t + 5;
      else cooldownUntil = t + 2;
    }
...
export async function fetchSignals(tickerCode = 'STX40.JO') {
  ...
  const { Q, states, rlEquity, trades } = runQLearning(aligned, regimes, rsiAligned, macdAligned.histogram, atrAligned, adxAligned);
  ...
  // ADX trend quality: ADX > 25 means strong directional trend (avoid choppy markets)
  factors.push({
    name: 'ADX Trend Quality',
    description: 'ADX above 25 (strong directional trend)',
    passed: latestADX >= 25,
  ...
  if (confluence) {
    ...
    for (let i = 0; i < alignedLen - 30; i++) {
      ...
      const pastScore = [pastBull, pastRSI >= 40 && pastRSI <= 70, pastMACDPos, pastBBOk, pastAboveSMA, pastADXStrong].filter(Boolean).length;
      if (pastScore >= 5) {
        total++;
